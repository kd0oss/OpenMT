...
template <typename TDATATYPE> bool CRingBuffer<TDATATYPE>::npeek(TDATATYPE& item, size_t pos) volatile
{
  if (m_head == m_tail && !m_full)
    return false;

  if((m_tail + pos) >= m_length)
  {
    if (((m_tail + pos) - m_length) >= m_head)
      return false;
    else
      item = m_buffer[(m_tail + pos) - m_length];
  }
  else
    item =  m_buffer[m_tail + pos];
