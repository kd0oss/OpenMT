...
char currentP25Refl[80] = "";
char rptrCallsign[20]   = "N0CALL";

/* C RingBuffer - for consistency */
RingBuffer reflTxBuffer;
RingBuffer txBuffer;

/* Mutexes protecting shared RingBuffers */
        ...
        if (loop > 1000)
        {
            ...
            if (avail >= 5)
            {
                ...
                if (buf[0] != 0x61)
                {
                    ...
                    /* Consume the bad byte so we don't loop forever */
                    pthread_mutex_lock(&txBufMutex);
...
bool writeLDU1(const uint8_t* ldu1, bool end)
{
    ...
    RingBuffer_addData(&reflTxBuffer, buffer, 23U);
    ...
    pthread_mutex_lock(&reflTxBufMutex);
    RingBuffer_addData(&reflTxBuffer, buffer, 15U);
    pthread_mutex_unlock(&reflTxBufMutex);
    ...
    pthread_mutex_lock(&reflTxBufMutex);
    RingBuffer_addData(&reflTxBuffer, buffer, 18U);
    pthread_mutex_unlock(&reflTxBufMutex);
    ...
    pthread_mutex_lock(&reflTxBufMutex);
    RingBuffer_addData(&reflTxBuffer, buffer, 17U);
    pthread_mutex_unlock(&reflTxBufMutex);
...
void createHeader(const uint8_t* data)
{
    ...
    pthread_mutex_lock(&txBufMutex);
    RingBuffer_addData(&txBuffer, header, P25_HDR_FRAME_LENGTH_BYTES + 8U);
    pthread_mutex_unlock(&txBufMutex);
}
...
void createNetLDU1(uint8_t* data)
{
    ...
    // m_netFrames += 9U;
    pthread_mutex_lock(&txBufMutex);
    RingBuffer_addData(&txBuffer, ldu, P25_LDU_FRAME_LENGTH_BYTES + 8U);
    pthread_mutex_unlock(&txBufMutex);
...
void createNetTerminator()
{
    ...
    pthread_mutex_lock(&txBufMutex);
    RingBuffer_addData(&txBuffer, data, P25_TERM_FRAME_LENGTH_BYTES + 8U);
    pthread_mutex_unlock(&txBufMutex);
    txOn = false;
}
...
void* connectP25Refl(void* argv)
{
    ...
    strncpy(rptrCallsign, source, sizeof(rptrCallsign) - 1);
    rptrCallsign[sizeof(rptrCallsign) - 1] = '\0';
    ...
    if (n == 11)
    {
        ...
        RingBuffer_addData(&txBuffer, (uint8_t*)P25Name, 6);
        pthread_mutex_unlock(&txBufMutex);
    }
    else
    {
    ...
    while (p25ReflConnected)
    {
        ...
        if (reflAvail >= 14)
        {
            ...
            pthread_mutex_lock(&reflTxBufMutex);
            RingBuffer_peek(&reflTxBuffer, hdr, 1);
            reflAvail = RingBuffer_dataSize(&reflTxBuffer);
            pthread_mutex_unlock(&reflTxBufMutex);

            if (reflAvail >= hdr[0])
            {
                pthread_mutex_lock(&reflTxBufMutex);
                RingBuffer_getData(&reflTxBuffer, buf, hdr[0]);
                pthread_mutex_unlock(&reflTxBufMutex);

                n = sendto(sockfd, buf + 1, hdr[0] - 1, 0, (struct sockaddr*)&serveraddr, sizeof(serveraddr));
                if (n < 0)
                    error((char*)"ERROR in sendto");
            }
        }
    ...
    while (connected)
    {
        ...
        if (memcmp(type, TYPE_COMMAND, typeLen) == 0)
        {
            if (buffer[8] == COMM_UPDATE_CONF)
            {
                ...
                pthread_mutex_lock(&txBufMutex);
                RingBuffer_addData(&txBuffer, buf, 8);
                pthread_mutex_unlock(&txBufMutex);
            }
        }
