...
#include "M17Golay.hpp"
#include "M17FrameDecoder.hpp"
#include "M17Interleaver.hpp"
#include "M17Decorrelator.hpp"
#include "M17CodePuncturing.hpp"
...
M17FrameType M17FrameDecoder::decodeFrame(const frame_t& frame)
{
    ...
    switch(type)
    {
        case M17FrameType::LINK_SETUP:
            bitErr = decodeLSF(data);
            break;

        case M17FrameType::STREAM:
            bitErr = decodeStream(data);
            break;

        case M17FrameType::PACKET:
            bitErr = decodePacket(data);
            break;
...
int M17FrameDecoder::decodeLSF(const std::array< uint8_t, 46 >& data)
{
    std::array< uint8_t, sizeof(M17LinkSetupFrame) > tmp;

    int err = viterbi.decodePunctured(data, tmp, LSF_PUNCTURE);
    memcpy(&lsf.data, tmp.data(), tmp.size());
    return err;
}

int M17FrameDecoder::decodePacket(const std::array< uint8_t, 46 >& data)
{
    ...
    int err = viterbi.decodePunctured(punctured, tmp, PACKET_PUNCTURE);
...
int M17FrameDecoder::decodeStream(const std::array< uint8_t, 46 >& data)
{
    ...
    int err = viterbi.decodePunctured(punctured, tmp, DATA_PUNCTURE);
